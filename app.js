...
function renderImportResult(ranges, missingItems = [], discrepancies = []) {
    ...
    // AUTO SAVE REMOVED: Prevent duplicate batches when uploading multiple files.
    // User must click the Save button manually.
}
...
function saveImportedBatch(isAuto = false) {
    ...
    if (importUpload) importUpload.value = '';
    document.getElementById('import-preview').classList.add('hidden');
    currentImportedBatches = [];
    rawTrackingData = []; // CRITICAL: Clear raw data to prevent duplicates on next upload
    importedFileCount = 0; // Reset file limit count
