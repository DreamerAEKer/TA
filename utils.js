 ...
 */
function getWeightFromPriceA3(priceWithSurcharge) {
    if (!priceWithSurcharge || priceWithSurcharge <= 0) return '-';

    // Remove the 3 baht surcharge to get the base Package A3 price for calculation
    const basePrice = priceWithSurcharge - 3;

    // Up to 10kg
    if (basePrice >= 19 && basePrice <= 109) {
        let w = (basePrice - 9) / 10;
        if (Number.isInteger(w)) return `${w} กก`;
    }
    // 11kg to 20kg
    if (basePrice >= 114 && basePrice <= 159) {
        let w = (basePrice - 59) / 5;
        if (Number.isInteger(w)) return `${w} กก`;
    }
    // > 20kg
    if (basePrice > 159) {
        let excess = (basePrice - 159) / 15;
