...
async function uploadToFirebase() {
    if (!currentImportedBatches || currentImportedBatches.length === 0) {
        alert("ไม่มีข้อมูลที่จะส่ง");
        return;
    }
    const batchName = document.getElementById('import-batch-name').value.trim() || `Batch_${new Date().getTime()}`;
    ...
    try {
        if (!window.db) throw new Error("Firebase is not initialized. Please check firebase_config.js.");
        ...
        const payload = {
            ...
            discrepancies: lastDiscrepanciesList || []
        };
        
        await window.db.collection("imported_batches").add(payload);
        
        alert("อัปโหลดขึ้น Firebase สำเร็จ!");
        clearImportData();
    } catch (err) {
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> ส่งข้อมูลขึ้นคลาวด์ (Send to Cloud)';
    }
}
